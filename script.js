...
if (videoPlayer) {
  ...
  const videoCoverImage = videoPlayer.querySelector(".video-cover-image");
  const videoId = videoPlayer.dataset.videoId;
  const videoStart = videoPlayer.dataset.videoStart || "0";

  videoCoverImage.addEventListener("error", () => {
    videoCoverImage.src = `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
  }, { once: true });

  videoCover.addEventListener("click", () => {
    const iframe = document.createElement("iframe");
    iframe.src = `https://www.youtube-nocookie.com/embed/${videoId}?start=${videoStart}&rel=0&autoplay=1`;
    iframe.title = videoCover.getAttribute("aria-label");
