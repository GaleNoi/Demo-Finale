...
if (calendarGrid && calendarMonth && calendarEvents && calendarPrev && calendarNext) {
  const initialCalendarTarget = recruitmentState().target;
  ...
  function renderCalendar() {
    ...
    const monthEvents = ranges
      .flatMap(range => [
        { date: range.start, label: "Apertura candidature" },
        { date: range.end, label: "Chiusura candidature" }
      ])
    ...
    if (!monthEvents.length) {
      ...
      emptyMessage.textContent = "Nessun appuntamento di recruitment previsto in questo mese.";
      calendarEvents.append(emptyMessage);
    } else {
