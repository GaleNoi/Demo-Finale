...
const patternLayers = [...document.querySelectorAll(".section-pattern")];
let ticking = false;
...
function renderScrollScene() {
  ...
  if (!reducedMotion) {
    patternLayers.forEach((layer, index) => {
      const localOffset = scrollY - layer.parentElement.offsetTop;
      layer.style.transform = `translate3d(0,${localOffset * (.045 + index * .02)}px,0) rotate(${progress * (index ? -4 : 4)}deg)`;
    });
  }
  ticking = false;
}
